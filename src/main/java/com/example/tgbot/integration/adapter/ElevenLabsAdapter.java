...
/**
 * Адаптер для ElevenLabs Text-to-Dialogue V3 API (kie.ai).
 * Вызывается только из web-интерфейса.
 */
@Service
@RequiredArgsConstructor
@Slf4j
public class ElevenLabsAdapter implements IRequestAdapter {
    @Getter
    ...
    public Optional<String> makeRequest(UserSession session) {
        ...
        if (options == null) {
            log.error("ElevenLabs: no options for model");
            return Optional.empty();
        }
        String fullCallbackUrl = baseUrl + endpointVersion + "/callbacks/elevenlabs-v3";
        ...
        try {
            ...
            log.trace("ElevenLabs response: {}", response);
            ...
            if (taskResponse.getCode() != 200 || taskResponse.getData() == null) {
                log.error("Kei AI createTask (ElevenLabs) failed: code={}, msg={}",
                        taskResponse.getCode(), taskResponse.getMessage());
                return Optional.empty();
            }
            String taskId = taskResponse.getData().getTaskId();
            if (taskId == null || taskId.isBlank()) {
                log.error("Kei AI createTask (ElevenLabs): taskId is empty");
                return Optional.empty();
            }
            ...
            log.error("Error during ElevenLabs request: {}", e.getMessage());
            return Optional.empty();
        }
    }
}
