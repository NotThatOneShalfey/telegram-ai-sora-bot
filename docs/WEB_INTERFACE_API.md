...
| HTTP | Код | Описание |
|------|-----|----------|
| 402 | E004 | Недостаточный баланс |
| 400 | E005, E006 | Невалидный запрос, ошибка загрузки файлов |
| 429 | E009 | Превышен лимит запросов |
| 502 | E001, E002, E003, E007, E010, E011 | Ошибка Kei AI или внешнего API |
...
| Недостаточно баланса | `SubmitOutcome.fail(E004)` | **402** — `{ "code": "E004", "description": "Недостаточный баланс" }` |
| Kei AI вернул code≠200 или ошибку при создании задачи | `SubmitOutcome.fail(E007)` | **502** — `{ "code": "E007", "description": "Ошибка внешнего API (Kei AI)" }` |
| Необработанное исключение в цепочке | `SubmitOutcome.fail(E008)` | **500** — `{ "code": "E008", "description": "Внутренняя ошибка сервера" }` |
...
| ErrorCode | HTTP | Тело |
|-----------|------|------|
| E004 | 402 | `{ "code": "E004", "description": "Недостаточный баланс" }` |
| E005, E006 | 400 | `{ "code": "E005/E006", "description": "..." }` |
| E009 | 429 | `{ "code": "E009", "description": "Превышен лимит запросов" }` |
| E001, E002, E003, E007, E010, E011 | 502 | `{ "code": "E00X", "description": "..." }` |
| E008 | 500 | `{ "code": "E008", "description": "Внутренняя ошибка сервера" }` |
...
| resultUrls | Массив объектов. Для Kling/Sora/NanoBanana — `[{url}]`; для SUNO_V5 — `[{audioUrl, imageUrl, title, text}]`. Пустой массив, если не применимо. |
...
| 502 | Ошибка внешнего API / Kei AI (E001, E002, E003, E007, E010, E011) |
